...
shared_examples 'a FR object' do
  it "can be created by a fundraiser" do
    expect(@ability).to be_able_to(:create, klass)
  end
  ...
  it "cannot be created by a sponsor" do
    ...
    expect(ability).to_not be_able_to(:create, klass)
  end
...
shared_examples 'an owned object' do
  context 'actions by the owner' do
    ...
    it "can update his own object" do
      expect(@ability).to be_able_to(:update, owned_object)
    end

    it "can destroy his own object" do
      expect(@ability).to be_able_to(:destroy, owned_object)
    end
  end

  context 'actions by foreign user' do
    it "cannot edit someone else's object" do
      expect(@ability).to_not be_able_to(:edit, foreign_object)
    end

    it "cannot update someone else's object" do
      expect(@ability).to_not be_able_to(:update, foreign_object)
    end

    it "cannot destroy someone else's object" do
      expect(@ability).to_not be_able_to(:destroy, foreign_object)
    end
  end
end
  ...
  describe 'Sponsor' do
    ...
    context 'Pledges' do
      it "can read pledges" do
        ...
        @pledges.each do |p|
          expect(@ability).to be_able_to(:read, p)
        end
      end
      ...
      context 'select_campaign' do
        it "can select a campaign to start a pledge" do
          ...
          expect(@ability).to be_able_to(:launch, @pledge)
        end
      end
      ...
      context 'launch' do
        ...
        it "cannot launch someone else's pledges" do
          ...
          expect(@ability).to_not be_able_to(:launch, @pledge)
        end
      end
      ...
      context 'increase' do
        it "can increase his own pledges" do
          @pledge = FactoryGirl.create(:pledge, sponsor: @sponsor)
          expect(@ability).to be_able_to(:increase, @pledge)
        end

        it "cannot increase someone else's pledges" do
          @pledge = FactoryGirl.create(:pledge)
          expect(@ability).to_not be_able_to(:increase, @pledge)
        end

        it "can set_increase his own pledges" do
          @pledge = FactoryGirl.create(:pledge, sponsor: @sponsor)
          expect(@ability).to be_able_to(:set_increase, @pledge)
        end

        it "cannot set_increase someone else's pledges" do
          @pledge = FactoryGirl.create(:pledge)
          expect(@ability).to_not be_able_to(:set_increase, @pledge)
        end
      end
    end
    ...
    context 'Pledge Requests' do
      ...
      it "can accept his pledge requests" do
        ...
        expect(@ability).to be_able_to(:accept, @pledge)
      end

      it "can reject his pledge requests" do
        @pledge = FactoryGirl.create(:pledge_request, sponsor: @sponsor)
        expect(@ability).to be_able_to(:reject, @pledge)
      end

      it "cannot accept someone else's pledge requests" do
        @pledge = FactoryGirl.create(:pledge_request)
        expect(@ability).to_not be_able_to(:accept, @pledge)
      end

      it "cannot reject someone else's pledge requests" do
        @pledge = FactoryGirl.create(:pledge_request)
        expect(@ability).to_not be_able_to(:reject, @pledge)
      end
    end
  end
  ...
  describe "Fundraiser" do
    ...
    context 'Campaigns' do
      ...
      it "can launch his campaign" do
        ...
        expect(@ability).to be_able_to(:launch, @campaign)
      end

      it "cannot launch someone else's campaign" do
        @campaign = FactoryGirl.create(:campaign)
        expect(@ability).to_not be_able_to(:launch, @campaign)
      end

      it "can save for later his campaign" do
        @campaign = FactoryGirl.create(:campaign, fundraiser: @fundraiser)
        expect(@ability).to be_able_to(:save_for_launch, @campaign)
      end

      it "cannot save for later someone else's campaign" do
        @campaign = FactoryGirl.create(:campaign)
        expect(@ability).to_not be_able_to(:save_for_launch, @campaign)
      end
    end
    ...
    context 'Pledges' do
      ...
      it "can add a message when rejecting own pledges" do
        @pledge = FactoryGirl.create(:pending_pledge, fundraiser: @fundraiser)
        expect(@ability).to be_able_to(:add_reject_message, @pledge)
      end

      it "cannot add a message when rejecting someone else's pledges" do
        @pledge = FactoryGirl.create(:pending_pledge)
        expect(@ability).to_not be_able_to(:add_reject_message, @pledge)
      end

      it "can create a pledge increase request" do
        @pledge = FactoryGirl.create(:pledge, fundraiser: @fundraiser)
        expect(@ability).to be_able_to(:increase_request, @pledge)
      end

      it "cannot create a increase request in someone else's pledges" do
        @pledge = FactoryGirl.create(:pledge)
        expect(@ability).to_not be_able_to(:increase_request, @pledge)
      end

      it "can delete own pledges" do
        @pledge = FactoryGirl.create(:pledge, fundraiser: @fundraiser)
        expect(@ability).to be_able_to(:destroy, @pledge)
      end

      it "cannot delete someone else's pledges" do
        @pledge = FactoryGirl.create(:pledge)
        expect(@ability).to_not be_able_to(:destroy, @pledge)
      end
    end
  end
    ...
    context 'actions' do
      describe "#badge" do
        ...
        it "should be available for a sponsor" do
          ability = Ability.new(sponsor.manager)
          expect(ability).to be_able_to(:badge, campaign)
        end
  ...
  describe "Pledges" do
    context 'actions' do
      describe "#badge" do
        ...
        it "should be available for a sponsor" do
          ability = Ability.new(sponsor.manager)
          expect(ability).to be_able_to(:badge, pledge)
        end
      ...
      describe "#click" do
        ...
        it "should be available for a sponsor" do
          ability = Ability.new(sponsor.manager)
          expect(ability).to be_able_to(:click, pledge)
        end
